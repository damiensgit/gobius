...
**Important Note:** While Gobius runs on Windows, Linux, and macOS, this guide primarily assumes you are setting up the miner on a **Linux server**, preferably a recent Ubuntu release hosted by a cheap cloud provider e.g. Vultr,  Hetzner and so on.
...
2. Edit `config.json` with your settings:
   ...
   **Understanding the Required Accounts**
   ...
   1.  **Main Operational Account (`privatekey`)**: 
        -   **Location**: The top-level `privatekey` field in `config.json`.
        -   **Purpose**: This account is primarily used to **pay gas fees (in ETH)** for various transactions initiated by Gobius, such as topping up validator stake, submitting bulk task commitments/claims, and potentially other administrative actions. It needs a sufficient ETH balance to cover these ongoing gas costs.

   2.  **Validator Account(s) (`validator_config.private_keys`)**:
        -   **Location**: The `private_keys` array within the `validator_config` section.
        -   **Purpose**: These accounts hold the required **AIUS stake** for mining. They are used to sign the solutions/claims generated by the miner and are the recipients of AIUS mining rewards. Each validator needs the minimum AIUS stake plus a recommended buffer (as detailed later). While they primarily hold AIUS, having a tiny amount of ETH might be useful in rare edge cases, but gas is typically paid by the Main Operational Account.
   ...
   3.  **Bulk Task Submission Account(s) (`batchtasks.private_keys`)**: 
        -   **Purpose**: Primarily for managing transaction nonces during high-volume operations (many GPUs submitting tasks concurrently). Sending many transactions quickly from one account can cause nonce issues.
        -   **Configuration**: The `config.json` includes a dedicated `batchtasks` section containing a `private_keys` array (e.g., `"batchtasks": { "private_keys": [] }`).
        -   **Behavior**: 
            -   **If Configured**: If you add one or more private keys (without `0x` prefix) to the `batchtasks.private_keys` array, Gobius will use **only these accounts** for sending bulk task/claim transactions, potentially cycling through them to help manage nonces.
            -   **If Empty (Default)**: If the `batchtasks.private_keys` array is left **empty** (`[]`), Gobius will **default** to using the **Main Operational Account** (specified by the `blockchain.private_key` field) for sending these bulk transactions.
        -   **Recommendation**: For most users, especially those starting or with fewer GPUs, the default behavior is recommended. Leave the `batchtasks.private_keys` array empty (`[]`) and let Gobius use the main `blockchain.private_key`. Configuring dedicated keys in `batchtasks.private_keys` is an advanced optimization for large-scale miners facing nonce limitations.
